...
## Update Summary
**Changes Made**
- Enhanced authentication flow with centralized auth.service.ts implementation
- Improved error handling with structured ApiError responses
- Better integration with new service architecture and type safety
- Updated frontend authentication components to use new service layer
...
**Updated** Enhanced with centralized authentication service layer, improved error handling, and better integration with the new service architecture.

## Project Structure
Authentication spans backend services and controllers, middleware, and frontend state management and UI components. The backend exposes REST endpoints for registration and login, while the frontend manages user sessions and navigates protected areas through a centralized service layer.
...
- Frontend auth service: encapsulates API calls to login/register endpoints with centralized error handling.
- Frontend API service: handles HTTP requests with automatic token injection and response parsing.
...
**Updated** Enhanced with centralized authentication service layer and improved error handling patterns.
...
- Registration/Login requests from the frontend to backend endpoints through a centralized service layer.
...
SVC->>API : "apiPost with token injection"
...
FE->>API : "Protected GET /owner/my-courts"
API->>M : "authenticate()"
M->>JWT : "verifyToken()"
...
- On failure, forwards an unauthorized error using structured ApiError.
...
- **Updated** Enhanced with centralized auth.service.ts providing unified API access patterns.
- Handles all authentication operations with proper error handling and type safety.
- Supports user registration, login, and owner registration with form data handling.

```mermaid
flowchart TD
Client["AuthClient.tsx"] --> Service["auth.service.ts"]
Service --> API["api.ts"]
API --> Backend["Backend Endpoints"]
Service --> Login["loginUser()"]
Service --> RegisterUser["registerUser()"]
Service --> RegisterOwner["registerOwner()"]
Login --> Transform["Transform contact fields"]
RegisterUser --> Transform
RegisterOwner --> FormData["FormData processing"]
```
...
- **Updated** Structured error handling using ApiError class for consistent error responses.
- Comprehensive type definitions for authentication requests and responses.
- Frontend authentication components now include robust error display and user feedback.
...
- **Updated** Enhanced dependency chain with centralized service layer integration.
...
- **Updated** Centralized service layer reduces redundant API calls and improves error handling performance.
...
- **Updated** Centralized service errors
  - Check auth.service.ts for proper error handling and response formatting.
  - Verify api.ts correctly injects Authorization headers for protected routes.
  - Ensure frontend components properly handle and display error messages.

- **Updated** Type safety issues
  - Verify auth.types.ts interfaces match backend response structures.
  - Check that FormData is properly constructed for owner registration.
...
**Updated** The enhanced authentication flow with centralized service layer provides improved error handling, better integration with the new service architecture, and more robust type safety across the frontend authentication components.