...
export class BookingService {
    async TaoDonDatSan(data, ipAddr = '127.0.0.1') {
        ...
        try {
            ...
            if (mappedPayment === "VNPay") {
                try {
                    ...
                    const paymentUrl = thanhToanService.TaoUrlThanhToanVNPay(ma_dat_san, tongTienCoc, orderInfo, ipAddr);
    ...
    async LayDatSanNguoiDung(userId) {
        if (!userId) {
            throw new ApiError(400, "User ID is required");
        }
        return await bookingRepository.TimTheoNguoiDung(userId);
    }
    async HuyDatSan(bookingId, userId) {
        if (!bookingId || !userId) {
        ...
        const tongTienCoc = booking.datsanchitiet.reduce((sum, d) => sum + Number(d.tien_coc), 0);
...
//# sourceMappingURL=datsan.service.js.map