...
export const DangKyChuSan = async (req, res, next) => {
    try {
...
export const LaySanCuaToi = async (req, res, next) => {
    try {
...
export const ThemSan = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const data = req.body;
        const files = req.files;
...
export const CapNhatSan = async (req, res, next) => {
    try {
        const userId = req.user.id;
        ...
        const images = files && files.length > 0
            ? files.map(f => ({ url: f.path, public_id: f.filename }))
            : undefined;
...
export const XoaSan = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const ma_san = req.params.ma_san;
        if (!ma_san) {
...
export const LayLichDatCuaToi = async (req, res, next) => {
    try {
        const userId = req.user.id;
...
export const CapNhatTrangThaiDatSan = async (req, res, next) => {
    try {
        const userId = req.user.id;
...
export const LaySoLuongChoXuLy = async (req, res, next) => {
    try {
        const userId = req.user.id;
...
//# sourceMappingURL=chusan.controller.js.map